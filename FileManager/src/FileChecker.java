        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(file.toString()))) {
            ...
            while ((line = reader.readLine()) != null) {
                if (line.contains("require")) {
                    if (new File(refactorString(line, directory)).isFile()) {
                        nodeList.add(new Node(refactorString(line, directory)));
                    } else {
                        System.out.println("Был обнаружен некорректный require! Файл указанный в строке с require не существует и не будет учтен в дальнейшей работе программы.");
                    }
                }
            }
        } catch (IOException e) {
